...
#if defined(NDEBUG)
# error "eDogCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x32c7a81888dbc007eebfc47f43e7dad70c1558f9976746cd888303d0f7664cf4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eDogCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eDogCoin Signed Message:\n";
...
{
    // eDogCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 2018 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 4252245 * COIN;
            }

    // Subsidy is cut in half every 20018 blocks
    nSubsidy >>= (nHeight / 20018);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // eDogCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // eDogCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1514237129, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "VOA NEWS 12 26 2017  Chinas Military Flybys Raise Alarm in Taiwan";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2018 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0454b3d7450b85b7ea9e45d29822e4f7575c9e34e18bf19b26987e165fa7038447877d2f36dbafbeadf5a0cca01704bf59b2c89d79d1691c393f4f133b6cfd15a5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514237129;
        ...
        assert(block.hashMerkleRoot == uint256("0x444c523ca6ce5697e1f066e71eb2ca9b897de57b2e29e642d10b52eedbcc26f9"));
...
unsigned char pchMessageStart[4] = { 0xf6, 0xa4, 0xf6, 0xad }; // eDogCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eDogCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eDogCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eDogCoinMiner(CWallet *pwallet)
{
    printf("eDogCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("edogcoin-miner");
    ...
    try { loop {
        ...
        printf("Running eDogCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("eDogCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eDogCoinMiner, pwallet));
}
